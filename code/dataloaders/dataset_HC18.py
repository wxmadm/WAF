...
class BaseDataSets(Dataset):
    def __init__(self, base_dir=None, labeled_type="labeled", labeled_ratio=10, split='train', transform=None, fold=1, cross_val=True):
        ...
        train_files = set(os.listdir("/home/wxm/code/SSLM4-LIS/data/HC18/train"))
        val_files = set(os.listdir("/home/wxm/code/SSLM4-LIS/data/HC18/val"))
        print("Train-Val Overlap:", train_files.intersection(val_files))
        if self.split == 'train':
            self.all_slices = os.listdir("/home/wxm/code/SSLM4-LIS/data/HC18/train_val")#去加载 他的切片，这个切片数据包含很多 ，所有train的 切片，有标签和无标签都包含进去
            self.sample_list = []
            labeled_ids = sorted(os.listdir(self._base_dir + "10/labeled"))
            unlabeled_ids = sorted(os.listdir(self._base_dir + "10/unlabeled"))
            ...
            # 加载所有的切片数据
            self.all_slices = os.listdir("/home/wxm/code/SSLM4-LIS/data/HC18/train_val")
            ...
            for ids in val_ids:
                new_data_list = list(
                    filter(lambda x: re.match('{}.*'.format(ids.replace(".h5", "")), x) is not None, self.all_slices))
                self.sample_list.extend(new_data_list)
